...
Jednak wraz z rozwojem Internetu coraz więcej z nich zaczęła przekształcać się w interaktywne aplikacje webowe, umożliwiające użytkownikowi wchodzenie z nimi w interakcje oraz komunikację z zewnętrznymi serwerami.
...
Na listingu \ref{lst:wasm} zaprezentowano przykładową funkcję obliczającą silnie napisaną w języku C oraz wynik jej kompilacji do WebAssembly.
...
Przedstawione poniżej prace przedstawiają aktualny stan wiedzy o WebAssembly oraz poruszają kwestie porównania wydajności WebAssembly z resztą dostępnych rozwiązań.
...
Prezentuje opis języka oraz wczesne testy wydajności, które pokazują dla niektórych testów wyniki zbliżone do prędkości natywnych aplikacji.
Podsumowany jest przyszłościowymi planami rozwoju technologii, m.in. udostępnieniem udogodnień znanych z języków programowania wysokiego poziomu jak np. obsługa wątków czy \emph{garbage collector}\footnotemark{}.
...
Otrzymane wyniki pokazały, że WebAssembly jest średnio o 30\% bardziej energooszczędny niż JavaScript.
WebAssembly poradził sobie również lepiej pod względem wydajności niż JavaScript.
...
\begin{itemize}
    ...
    \item \emph{RTCPeerConnection} -- uruchamia komunikację audiowizualną pomiędzy użytkownikami, odpowiada za przetwarzanie sygnału, obsługę kodeków, komunikację, bezpieczeństwo oraz zarządzanie prędkości połączenia
...
Przedstawione poniżej prace skupiają się na porównaniu technologii WebRTC oraz WebSocket.
...
Wykorzystał telefon jako bramkę IoT -- stanowiącą pośrednika między urządzeniami a Internetem.
...
Umożliwił on również komunikację aplikacji w obrębie sieci lokalnej\footnotemark{}, mimo postawienia strony na publicznym serwerze.
...
Praca \cite{websocket_webrtc_streamr} autorstwa \emph{Santeri Juslenius} analizowała zachowanie technologii WebRTC oraz WebSocket w sieci \emph{Streamr}, która jest zdecentralizowanym systemem \emph{publish-subscribe}\footnotemark{}.
...
Doszli do wniosku, że WebSocket oferuje lepsze rezultaty i uznali go za dobre rozwiązanie w aplikacjach komunikujących się w czasie rzeczywistym.
...
Pomimo dużej prostoty, serwer jest w pełni funkcjonalnym serwer sygnalizacyjnym WebRTC.
Umożliwia on realizacje komunikacji pomiędzy dwoma aplikacjami webowymi używającymi WebRTC.
...
Na podstawie eksperymentalnej aplikacji webowej uznali, że wykorzystanie technologii WebSocket umożliwia uzyskanie niższych opóźnień niż odpytywanie HTTP czy rozwiązania \emph{FlashSocket} i \emph{Socket}.
...
Zastosowanie WebAssembly w wielu przypadkach przyspieszyło działanie aplikacji i stosowanie go w obliczeniowo intensywnych partiach kodu jest wskazane.
